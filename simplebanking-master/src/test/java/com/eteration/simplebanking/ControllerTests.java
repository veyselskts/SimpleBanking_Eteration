...
class ControllerTests  {
    ...
    public void givenId_Credit_thenReturnJson() throws Exception {
        ...
        doReturn(new TransactionStatus("OK", "APPROVAL-CODE"))
                .when(service).credit(eq("17892"), any(DepositTransaction.class));
    ...
    throws Exception {
        ...
        doReturn(account).when(service).findAccount( "17892");
        doReturn(new TransactionStatus("OK", "APPROVAL-CODE"))
            .when(service).debit(eq("17892"), any(WithdrawalTransaction.class));
        ResponseEntity<TransactionStatus> result = controller.credit( "17892", new TransactionRequestDto(1000.0));
    ...
    throws Exception {
        //Assertions.assertThrows( InsufficientBalanceException.class, () -> {
            ...
            doReturn(new TransactionStatus("OK", "APPROVAL-CODE"))
                .when(service).credit(eq("17892"), any(DepositTransaction.class));
             doReturn(new TransactionStatus("FAIL", "Yetersiz bakiye"))
                .when(service).debit(eq("17892"), any(WithdrawalTransaction.class));
